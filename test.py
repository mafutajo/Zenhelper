...
def process_mail_list(input_path="Doc/mail_list.csv"):
    ...
    # === distinct_titles.csv ===
    unique_titles = sorted(df["title_cleaned"].dropna().unique())
    pd.DataFrame(unique_titles, columns=["title_cleaned"]).to_csv(
        "distinct_titles.csv", index=False
    )
    print("✅ distinct_titles.csv généré.")
